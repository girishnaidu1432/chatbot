...
# Helper: centralize the request processing so both manual input and preset buttons use same flow
def process_user_input(user_input_text):
    ...
    # Call to OpenAI (keeps original behavior)
    try:
    ...
    # Save user and assistant messages to state
    ...
    # Clear the preset selection after run (if any)
    if "_selected_preset" in st.session_state:
        del st.session_state["_selected_preset"]

    st.rerun()

# Chat input area (user typed)
user_input = st.chat_input("Enter your query:")

# If user typed and submitted a query
if user_input and user_input.strip():
    process_user_input(user_input)

# If a preset prompt was selected via sidebar button, process it here
if st.session_state.get("_selected_preset"):
    selected = st.session_state.get("_selected_preset")
    if selected:
